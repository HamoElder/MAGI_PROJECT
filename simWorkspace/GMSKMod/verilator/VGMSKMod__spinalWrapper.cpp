...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.mod_iq_valid );
      signalAccess[1] = new CDataSignalAccess( top.mod_iq_payload_last );
      signalAccess[2] = new SDataSignalAccess( top.mod_iq_payload_fragment_cha_i );
      signalAccess[3] = new SDataSignalAccess( top.mod_iq_payload_fragment_cha_q );
      signalAccess[4] = new CDataSignalAccess( top.unit_data_valid );
      signalAccess[5] = new CDataSignalAccess( top.unit_data_payload_last );
      signalAccess[6] = new CDataSignalAccess( top.unit_data_payload_fragment );
      ...
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/missdown/IdeaProjects/MAGI_PROJECT/./simWorkspace/GMSKMod/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("/home/missdown/IdeaProjects/MAGI_PROJECT/./simWorkspace/GMSKMod/") + name + ".dat").c_str());
      #endif
    }
