...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.cp_len );
      signalAccess[1] = new CDataSignalAccess( top.data_len );
      signalAccess[2] = new CDataSignalAccess( top.raw_data_valid );
      signalAccess[3] = new CDataSignalAccess( top.raw_data_ready );
      signalAccess[4] = new CDataSignalAccess( top.raw_data_payload_last );
      signalAccess[5] = new IDataSignalAccess( top.raw_data_payload_fragment_cha_i );
      signalAccess[6] = new IDataSignalAccess( top.raw_data_payload_fragment_cha_q );
      signalAccess[7] = new CDataSignalAccess( top.result_valid );
      signalAccess[8] = new CDataSignalAccess( top.result_payload_last );
      signalAccess[9] = new IDataSignalAccess( top.result_payload_fragment_cha_i );
      signalAccess[10] = new IDataSignalAccess( top.result_payload_fragment_cha_q );
      ...
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/missdown/MAGI_PROJECT/./simWorkspace/CyclicPrefix/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("/home/missdown/MAGI_PROJECT/./simWorkspace/CyclicPrefix/") + name + ".dat").c_str());
      #endif
    }
