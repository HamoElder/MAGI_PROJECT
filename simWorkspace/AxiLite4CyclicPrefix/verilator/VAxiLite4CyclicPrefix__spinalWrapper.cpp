...
public:
    ...
    uint32_t timeCheck;
    bool waveEnabled;
    VAxiLite4CyclicPrefix top;
    ISignalAccess *signalAccess[32];
    #ifdef TRACE
	  VerilatedVcdC tfp;
	  #endif
    string name;
    ...
    Wrapper_1(const char * name){
      ...
      signalAccess[19] = new CDataSignalAccess( top.raw_data_valid );
      signalAccess[20] = new CDataSignalAccess( top.raw_data_ready );
      signalAccess[21] = new CDataSignalAccess( top.raw_data_payload_last );
      signalAccess[22] = new SDataSignalAccess( top.raw_data_payload_fragment_cha_i );
      signalAccess[23] = new SDataSignalAccess( top.raw_data_payload_fragment_cha_q );
      signalAccess[24] = new CDataSignalAccess( top.result_valid );
      signalAccess[25] = new CDataSignalAccess( top.result_payload_last );
      signalAccess[26] = new SDataSignalAccess( top.result_payload_fragment_cha_i );
      signalAccess[27] = new SDataSignalAccess( top.result_payload_fragment_cha_q );
      signalAccess[28] = new CDataSignalAccess( top.rf_clk );
      signalAccess[29] = new CDataSignalAccess( top.rf_resetn );
      signalAccess[30] = new CDataSignalAccess( top.clk );
      signalAccess[31] = new CDataSignalAccess( top.reset );

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/missdown/MAGI_PROJECT/./simWorkspace/AxiLite4CyclicPrefix/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("/home/missdown/MAGI_PROJECT/./simWorkspace/AxiLite4CyclicPrefix/") + name + ".dat").c_str());
      #endif
    }
