...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.data_valid );
      signalAccess[1] = new SDataSignalAccess( top.data_payload );
      signalAccess[2] = new CDataSignalAccess( top.phase_valid );
      signalAccess[3] = new SDataSignalAccess( top.phase_payload );
      signalAccess[4] = new CDataSignalAccess( top.channel_en );
      signalAccess[5] = new CDataSignalAccess( top.sync_en );
      signalAccess[6] = new SDataSignalAccess( top.phase_limit );
      signalAccess[7] = new SDataSignalAccess( top.phase_offset );
      signalAccess[8] = new SDataSignalAccess( top.phase_inc );
      ...
      top.trace(&tfp, 99);
      tfp.open((std::string("/home/missdown/MAGI_PROJECT/./simWorkspace/DDS/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("/home/missdown/MAGI_PROJECT/./simWorkspace/DDS/") + name + ".dat").c_str());
      #endif
    }
