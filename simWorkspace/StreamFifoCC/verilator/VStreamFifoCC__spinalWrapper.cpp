...
public:
    ...
    Wrapper_1(const char * name){
      ...
      waveEnabled = true;
      signalAccess[0] = new CDataSignalAccess( top.io_push_valid );
      signalAccess[1] = new CDataSignalAccess( top.io_push_ready );
      signalAccess[2] = new IDataSignalAccess( top.io_push_payload );
      signalAccess[3] = new CDataSignalAccess( top.io_pop_valid );
      signalAccess[4] = new CDataSignalAccess( top.io_pop_ready );
      signalAccess[5] = new IDataSignalAccess( top.io_pop_payload );
      signalAccess[6] = new CDataSignalAccess( top.io_pushOccupancy );
      signalAccess[7] = new CDataSignalAccess( top.io_popOccupancy );
      signalAccess[8] = new CDataSignalAccess( top.clkA_clk );
      signalAccess[9] = new CDataSignalAccess( top.clkA_reset );
      signalAccess[10] = new CDataSignalAccess( top.clkB_clk );

      #ifdef TRACE
      Verilated::traceEverOn(true);
      top.trace(&tfp, 99);
      tfp.open((std::string("/mnt/BackUp/IdeaProjects/MAGI_PROJECT/./simWorkspace/StreamFifoCC/") + name + ".vcd").c_str());
      #endif
      this->name = name;
    }
    ...
    virtual ~Wrapper_1(){
      ...
      #endif
      #ifdef COVERAGE
      VerilatedCov::write((("/mnt/BackUp/IdeaProjects/MAGI_PROJECT/./simWorkspace/StreamFifoCC/") + name + ".dat").c_str());
      #endif
    }
