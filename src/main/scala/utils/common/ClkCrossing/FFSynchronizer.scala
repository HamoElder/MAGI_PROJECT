...
case class FFSynchronizer[T <: Data](dataType: T, clkI : ClockDomain,clkO : ClockDomain) extends Component {
	val io = new Bundle{
...
object FFSynchronizer{
	def apply[T <: Data](clkI : ClockDomain,clkO : ClockDomain, data: T): T ={
		val clk_cross = FFSynchronizer(data, clkI = clkI, clkO = clkO)
