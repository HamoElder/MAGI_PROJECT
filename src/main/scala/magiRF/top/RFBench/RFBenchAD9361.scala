...
import magiRF.top.RFBench.Config.{axiLite4_config, cross_clk_fifo_depth, genPhyPkgConfig, genStreamPkgConfig, interfaceIQDataType, phyDataType, rf_payload_upper_boundary, stream_config, stream_data_type}
...
//        val recv_intr = out(Bool())
    ...
    val rx_fifo = StreamFifo(
        AxiStream4X(stream_config),
        rf_payload_upper_boundary.toInt
    )
    rx_fifo.io.push << rf_to_core_fifoCc.io.pop
    io.recv_data.stream << rx_fifo.io.pop
    ...
    val transmitter_bridge = rfTxClockArea.transmitter.driveFrom(axil4busCtrl, 0x10, this.clockDomain, rf_interface.txClockDomain)
    val receiver_bridge = rfRxClockArea.receiver.driveFrom(axil4busCtrl, 0x40, this.clockDomain, rf_interface.rxClockDomain)
