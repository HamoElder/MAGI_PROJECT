...
import magiRF.top.RFBench.Config.{codedDataWidth, genModulatorConfig, genModulatorDivConfig, interConnectFifoDepth,
    interConnectHaltThreshold, interfaceIQDataType, mask_seq_1_2, mod_method_type, phyDataType, stf_preamble_config}
...
case class TX() extends Component{
    ...
    phy_tx_information_gen.io.raw_data << io.raw_data
    ...
    phy_tx_crc.io.raw_data << information_to_crc_fifo._1.haltWhen(pipeline_halt(0))
    val crc_to_padder_fifo = phy_tx_crc.io.result_data.queueWithAvailability(interConnectFifoDepth)
    pipeline_halt(0) := crc_to_padder_fifo._2 < interConnectHaltThreshold
    val phy_tx_padder = PhyTxPadder()
    phy_tx_padder.io.raw_data << crc_to_padder_fifo._1.haltWhen(pipeline_halt(1))
    val padder_to_encoder_fifo = phy_tx_padder.io.result_data.queueWithAvailability(interConnectFifoDepth)
    pipeline_halt(1) := padder_to_encoder_fifo._2 < interConnectHaltThreshold
    val phy_tx_encoder = PhyTxEncoder()
    phy_tx_encoder.io.raw_data << padder_to_encoder_fifo._1.haltWhen(pipeline_halt(2))
    ...
    pipeline_halt(2) := puncher_to_scrambling_fifo._2< interConnectHaltThreshold
    val phy_tx_scrambler = PhyTxScrambler()
    phy_tx_scrambler.io.raw_data << puncher_to_scrambling_fifo._1.haltWhen(pipeline_halt(3))
    val scrambler_to_mod_data_div_fifo = phy_tx_scrambler.io.result_data.queueWithAvailability(interConnectFifoDepth)
    pipeline_halt(3) := scrambler_to_mod_data_div_fifo._2 < interConnectHaltThreshold
    val mod_data_div = dataDivDynamic(genModulatorDivConfig)
    mod_data_div.io.base_data << scrambler_to_mod_data_div_fifo._1.haltWhen(pipeline_halt(4))
    ...
    pipeline_halt(4) := mod_rtl_to_header_extender_fifo._2 < interConnectHaltThreshold
    val phy_header_extender = PhyHeaderExtender()
    phy_header_extender.io.raw_data << mod_rtl_to_header_extender_fifo._1.haltWhen(pipeline_halt(5))
    ...
    pipeline_halt(5) := header_extender_to_oversampling_fifo._2 < interConnectHaltThreshold
    val phy_tx_oversampling = PhyTxOverSampling()
    phy_tx_oversampling.io.raw_data << header_extender_to_oversampling_fifo._1.haltWhen(pipeline_halt(6))
