...
case class mFSKModExtension() extends ModExtension{
    override def needTag: Boolean = false
    override def getName: String = "mFSK_Modulator_Extension"

    override def applyIt(mod_rtl_core: ModulatorRTL, index: Int): Area = new Area{
        val mod = mFSKMod(mod_rtl_core.config.modConfig(index))
