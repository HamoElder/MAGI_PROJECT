...
case class mQAMModExtension() extends ModExtension{
    override def needTag: Boolean = false
    override def getName: String = "mQAM_Modulator_Extension"

    override def applyIt(mod_rtl_core: ModulatorRTL, index: Int): Area = new Area{
        val mod = mQAMMod(mod_rtl_core.config.modConfig(index))
