...
import utils.common.Mux.FlowDeMux


case class CDMA(iqWidth: Int, codeLength: Int, addrWidth: Int, useDynamic:Boolean = true, codeTable: Seq[Bits] = null) extends Component {
    ...
    val io = new Bundle{
        ...
        val w_addr = if(useDynamic) in(UInt(addrWidth bits)) else null
...
object CDMABench{
    def main(args: Array[String]): Unit = {
        SpinalConfig(defaultConfigForClockDomains = ClockDomainConfig(resetKind = SYNC, resetActiveLevel = LOW),
            targetDirectory = "rtl/CDMA").generateSystemVerilog(new CDMA(16, 8, 32, true)).printPruned()
    }
}
