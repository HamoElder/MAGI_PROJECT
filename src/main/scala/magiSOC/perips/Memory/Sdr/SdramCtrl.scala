        ...
        switch(state) {
            ...
            default { //RUN
                when(refresh.pending){
                    ...
                    } elsewhen(banks(address.bank).active === False) {
                        rsp.task := ACTIVE
                        val bank = banks(address.bank)
...
object SdramCtrlMain{
    def main(args: Array[String]): Unit = {
        ...
        SpinalConfig(defaultClockDomainFrequency = FixedFrequency(100 MHz)).generateSystemVerilog(SdramCtrl(device.layout,device.timingGrade7,3,UInt(8 bits)))
    }
}